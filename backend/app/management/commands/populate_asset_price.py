    ...
    def handle(self, *args, **options):
        ...
        def get_standard_lot(start, end):
            response = requests.request(
                "GET",
                f"https://arquivos.b3.com.br/bdi/table/StandardLot/{start}/{end}/1/2000",
            )
            if response.text:
                try:
            ...
            if data["table"] and data["table"]["values"]:
                for asset_data in data["table"]["values"]:
                    ...
                    if (
                        ...
                        and not AssetPrice.objects.filter(
                            ticker=ticker, date=end
                        ).exists()
                    ):
                        new_asset_price = AssetPrice(
                            ticker=ticker,
                            date=end,
                            open=open,
                            low=low,
        ...
        def get_real_estate_funds(start, end):
            response = requests.request(
                "GET",
                f"https://arquivos.b3.com.br/bdi/table/RealEstateFunds/{start}/{end}/1/2000",
            )
            if response.text:
                try:
                    data = json.loads(response.text)
        ...
        data_inicio = datetime(2024, 4, 20)
        data_fim = datetime(2024, 5, 2)
